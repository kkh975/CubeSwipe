...
( function( $ ) {
	 ...
	 * @method: 슬라이더 플러그인
	 */
	$.fn.slideCubeSwipe = function( option ) {
		...
		return this.each( function( ) {
			$( this ).data( 'scs', new SlideCubeSwipe( option ) );
		} );
	};
	 ...
	 */
	$.fn.slideCubeSwipe2start = function( ) {
		return this.each( function( ) {
	 ...
	 */
	$.fn.slideCubeSwipe2stop = function( ) {
		return this.each( function( ) {
	 ...
	 */
	$.fn.slideCubeSwipe2prev = function( ) {
		return this.each( function( ) {
	 ...
	 */
	$.fn.slideCubeSwipe2next = function( ) {
		return this.each( function( ) {
	 ...
	 */
	$.fn.slideCubeSwipe2toSlide = function( _idx ) {
		return this.each( function( ) {
 ...
 * @method: 사각형 플리킹 함수
 */
function SlideCubeSwipe( __setting ) {
	...
	var helperCss3 = { // css3 보조 함수
		 ...
		 * @method: 애니메이션 만들기
		 */
		getAnimateRuleTxt: function( _name, _from_angle, _to_angle ) {
			...
			while( --ani_idx > -1 ) {
				...
				while( --trn_idx > -1 ) {
					css_txt += trn_arr[ trn_idx ] + 'transform: ';
					css_txt += 'rotateY(' + _from_angle + 'deg) ';
					css_txt += 'translateZ(' + cube_Radius + 'px);';
				}
		 ...
		 * @method: Transit 설정
		 */
		setTransit: function( _dom, _angle, _time ) {
			var arr = TRANSITION_VENDORS,
				idx = arr.length,
				dom_style = _dom.style;

			while( --idx > -1 ) {
				dom_style.setProperty( arr[ idx ] + 'transform', 'rotateY('+ _angle + 'deg) translateZ(' + cube_Radius + 'px)' );
		 ...
		 */
		setTransitList: function( _is_set, _add_angle, _time ) {
			var angle = 0,
				len = 0,
				i = 0;
			...
			for ( i = 0; i < list_Len; i++ ) {
				...
				helperCss3.setTransit( list_Dom[ i ], angle, _time );
			}
		},
		...
		setEnd: function( e ) {
			...
			if ( touchEvents.is_touch_start && e.type === 'touchend' && e.touches.length === 0 ) {
				...
				if ( over_touch && can_move ) {
					...
					angle = is_to_next ? -BASE_ROTATE : BASE_ROTATE;
					idx = getIdx( );
				...
				helperCss3.setTransitList( true, angle, setting.duration );
	...
	function setInitStyle( ) {
		...
		// 페인트 타임으로 인하여, css3의 animate로 접근해야 함, 리스트 다음에 style 태그가 위치해야 함.
		css_txt = helperCss3.getAnimateRuleTxt( 'next-to-now', BASE_ROTATE, 0 );
		css_txt += helperCss3.getAnimateRuleTxt( 'now-to-prev', 0, -BASE_ROTATE );
		css_txt += helperCss3.getAnimateRuleTxt( 'prev-to-now', -BASE_ROTATE, 0 );
		css_txt += helperCss3.getAnimateRuleTxt( 'now-to-next', 0, BASE_ROTATE );
	...
	function update( _list_dom ) {
		...
		while( ++i < len ) {
			...
			// 스타일
			css_txt = 'position: absolute; width: 100%; height: 100%; ';
			css_txt += 'visibility: ' + ( 2 > i || list_Len - 1 === i ? 'visible' : 'hidden' );
			list_Dom[ i ].style.cssText = css_txt;
			list_Dom[ i ].setAttribute( 'data-mark', 's4s-item' );

			css_txt = 'rotateY(' + angle + 'deg) ';
			css_txt += 'translateZ(' + cube_Radius + 'px) ';
			helperCss3.setCommonRule( list_Dom[ i ], 'backfaceVisibility', 'visible', TRANSITION_VENDORS );
			helperCss3.setCommonRule( list_Dom[ i ], 'transform', css_txt, TRANSITION_VENDORS );
		...
		list_Angle_Arr = angle_arr.slice( );
		helperCss3.setTransitList( );
	}
	...
	function refreshSize( ) {
		...
		helper.setCss3( wrap_Dom, 'perspective', ( list_Width * 2 ) + 'px' );
	}
	...
	function setMoveAfter( ) {
		...
		if ( typeof setting.active === 'function' ) {
			setting.active( is_Loop_Len_2 ? getIdx( ) % 2 : getIdx( ) );
		}
	}
